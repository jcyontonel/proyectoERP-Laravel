...
<div class="min-h-screen bg-gray-100 p-6">
    ...
    <form id="venta-form" class="space-y-10 bg-white p-10 rounded-2xl shadow-2xl" 
        ...
        <div id="cliente-form" class="relative mb-4">
        ...
        <div id="producto-form" class="mb-8">
            ...
            <div class="relative mb-4">
                ...
                <ul id="producto-list" 
                    ...
                    class="absolute z-10 w-full bg-white border border-gray-300 rounded-2xl mt-2 
                            overflow-y-auto shadow-xl hidden"></ul>
            </div>
            <!-- / Producto Autocompletar -->
        ...
        <div class="flex gap-6 mb-10">
            ...
            <button type="button" id="btn-efectivo"
                class="flex-1 bg-green-600 font-bold rounded-2xl text-4xl px-8 py-8 text-center shadow-lg transition-all duration-300">
                💵 Efectivo
            </button>

            <button type="button" id="btn-yape"
                class="flex-1 bg-purple-600 font-bold rounded-2xl text-4xl px-8 py-8 text-center shadow-lg transition-all duration-300">
                📱 Yape
            </button>
        </div>
...
<style>
 #producto-list::-webkit-scrollbar {
    width: 20px;
}
#producto-list::-webkit-scrollbar-thumb {
...
<script>
    document.getElementById('cliente-form').hidden = true;
</script>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        inputProducto.addEventListener('input', function() {
            ...
            filtered.forEach(p => {
                const li = document.createElement('li');
                li.className = 'cursor-pointer px-6 py-12 text-5xl hover:bg-blue-100 flex justify-between items-center';
                li.innerHTML = `<span>${p.nombre}</span> <span class="ml-6 text-blue-700 text-3xl">S/.${p.precio_unitario.toFixed(2)}</span>`;
                li.dataset.id = p.id;
...
(() => {
    ...
    function seleccionarPago(pago) {
        console.log('Seleccionado pago:', pago);
        if (pago === 'efectivo') {
            metodoPagoInput.value = 'efectivo';
            btnEfectivo.classList.add('bg-green-600', 'shadow-2xl', 'scale-105');
            btnEfectivo.classList.remove('bg-green-200');
            btnYape.classList.add('bg-purple-200');
            btnYape.classList.remove('bg-purple-600', 'shadow-2xl', 'scale-105');
        } else {
            metodoPagoInput.value = 'yape';
            btnYape.classList.add('bg-purple-600', 'shadow-2xl', 'scale-105');
            btnYape.classList.remove('bg-purple-200');
            btnEfectivo.classList.add('bg-green-200');
            btnEfectivo.classList.remove('bg-green-600', 'shadow-2xl', 'scale-105');
        }
    }
    ...
    btnEfectivo.addEventListener('click', () => seleccionarPago('efectivo'));
    btnYape.addEventListener('click', () => seleccionarPago('yape'));
})();
</script>
